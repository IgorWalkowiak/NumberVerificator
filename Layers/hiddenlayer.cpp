...
HiddenLayer::HiddenLayer(int outputSize, LayerI *previousLayer,
  ...
  std::cout << "HiddenLayer Constructor inputData address: " << mInputData
            << "Input size: " << mInputData->size() << std::endl;
  weights =
      std::vector(mInputData->size(), std::vector<float>(mOutputData.size()));
  for (auto &vector : weights) {
    for (auto &weight : vector)
      weight = 3.0;
  }

  // std::cout<<"HiddenLayer constructor :: DEBUG
  // inputs="<<this->getPreviousLayerOutputSize()<<"
  // outputs="<<this->getOutputSize()<<std::endl;
}

HiddenLayer::HiddenLayer(int outputSize,
                         std::function<float(float)> activFunction)
...
void HiddenLayer::processInputs() {
  ...
  for (int outputIndex = 0; outputIndex < mOutputData.size(); outputIndex++) {
    ...
    for (int inputIndex = 0; inputIndex < mInputData->size(); inputIndex++) {
      // sume = sume + (weights[outputIndex][inputIndex] *
      // mInputData->at(inputIndex));
