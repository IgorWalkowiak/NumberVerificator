...
InputDataDeliver::InputDataDeliver(string csvFileDir) : csvParser(csvFileDir) {
  this->inputData.colors.resize(
      784); // looking for smart way to keep it flexible
...
void InputDataDeliver::changeInputData(int index) {
  std::cout << "___Number: " << (int)parsedCsvData.at(index).first
            << ", Path: " << parsedCsvData.at(index).second << std::endl;

  imagesParser.getDataFromImage(parsedCsvData.at(index).second,
                                this->inputData.colors);
